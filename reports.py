...
class Report:
    ...
    def phase_lane(self, num_phase):
        ...
        golds_bot_blue = (
            self.results["golds"][1][num_phase] + self.results["golds"][2][num_phase]
        )
        golds_bot_red = (
            self.results["golds"][4][num_phase] + self.results["golds"][5][num_phase]
        )
        ...
        if diff >= 0:
            st.subheader(
                f"""Golds :blue[{golds_total_blue}] vs :red[{golds_total_red}]  (+:blue[{diff}])"""
            )
        else:
            st.subheader(
                f"""Golds :blue[{golds_total_blue}] vs :red[{golds_total_red}]  (+:red[{abs(diff)}])"""
            )

        diff_mid = golds_mid_blue - golds_mid_red
        if diff_mid >= 0:
            st.write(
                f"""Mid :blue[{golds_mid_blue}] vs :red[{golds_mid_red}]  (+:blue[{diff_mid}])"""
            )
        else:
            st.write(
                f"""Mid :blue[{golds_mid_blue}] vs :red[{golds_mid_red}]  (+:red[{abs(diff_mid)}])"""
            )

        diff_bot = golds_bot_blue - golds_bot_red
        if diff_bot >= 0:
            st.write(
                f"""Bot :blue[{golds_bot_blue}] vs :red[{golds_bot_red}]  (+:blue[{diff_bot}])"""
            )
        else:
            st.write(
                f"""Bot :blue[{golds_bot_blue}] vs :red[{golds_bot_red}]  (+:red[{abs(diff_bot)}])"""
            )
    ...
    def drake(self):
        ...
        damage_data = pd.DataFrame(
            {
                "noms": list(degats.keys()),
                "damage": list(degats.values()),
        ...
        chart_df = (
            ...
            .mark_bar()
            .encode(
                x=alt.X("noms", sort=None), y=alt.Y("damage", sort=None), color="color"
            )
        )
    ...
    def simulation_game(self, num_fight):
        ...
        states = [
            st.session_state.state_drake,
            st.session_state.state_baron,
            st.session_state.state_ancestral,
        ]
            ...
            if num_tour < tour_engage:
                ...
                for b, blue_champ in enumerate(self.results["blue_team"]):
                    ...
                    if distances[b] is None:
                        ...
                        try:
                            st.image(
                                "data/images/" + blue_champ + ".webp",
                                use_column_width=True,
                            )
                        except Exception:
                            st.image(
                                "data/images/" + blue_champ + ".jpg",
                                use_column_width=True,
                            )
        ...
        with cols[-2]:
            ...
            if num_tour < tour_engage:
                ...
                for i in range(3):
                    ...
                    dfs[i] = pd.DataFrame(
                        {"hps": [fight_hps, hp_max[i + 3]]}, index=None
                    )
                    st.write(fight_hps)
                    st.bar_chart(dfs[i], height=180)
        ...
        with cols[-1]:
            ...
            if num_tour < tour_engage:
                ...
                for r, red_champ in enumerate(self.results["red_team"]):
                    ...
                    try:
                        st.image(
                            "data/images/" + red_champ + ".webp", use_column_width=True
                        )
                    except Exception:
                        st.image(
                            "data/images/" + red_champ + ".jpg", use_column_width=True
                        )
                ...
                for r, red_champ in enumerate(self.results["red_team"]):
                    ...
                    if distances[r + 3] is None:
                        ...
                        try:
                            st.image(
                                "data/images/" + red_champ + ".webp",
                                use_column_width=True,
                            )
                        except Exception:
                            st.image(
                                "data/images/" + red_champ + ".jpg",
                                use_column_width=True,
                            )
...
def main():
    ...
    if st.session_state.match_selected:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["Choix match", "Résumé", "Fight drake", "Fight baron", "Fight Ancestral"]
        )
